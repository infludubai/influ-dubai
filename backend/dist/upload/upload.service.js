...
let UploadService = UploadService_1 = class UploadService {
    ...
    async uploadFile(buffer, originalName, bucket) {
        const ext = path.extname(path.basename(originalName)).toLowerCase();
        const fileName = `${(0, crypto_1.randomUUID)()}${ext}`;
        const dir = path.join(this.root, bucket);
        try {
            await fs_1.promises.mkdir(dir, { recursive: true });
            await fs_1.promises.writeFile(path.join(dir, fileName), buffer);
        }
        catch (err) {
            this.logger.error(`Could not store upload: ${err.message}`);
