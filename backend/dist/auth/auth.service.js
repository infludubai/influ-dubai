...
let AuthService = AuthService_1 = class AuthService {
    ...
    constructor(prisma, jwt, config, mail, workspaces, settings) {
        this.prisma = prisma;
        this.jwt = jwt;
        this.config = config;
        this.mail = mail;
    ...
    async register(dto) {
        ...
        const requireApproval = this.settings.isOn('REQUIRE_SIGNUP_APPROVAL') && !bootstrappedAdmin;
        ...
        if (roleName === 'ADMIN') {
            this.logger.warn(`Bootstrapped ${user.email} as the first ADMIN via BOOTSTRAP_ADMIN_EMAIL. ` +
    ...
    async shouldBootstrapAsAdmin(email) {
        const configured = this.config.get('BOOTSTRAP_ADMIN_EMAIL')?.trim().toLowerCase();
    ...
    async login(dto) {
        ...
        if (user.status === 'PENDING_APPROVAL') {
            throw new common_1.ForbiddenException('Your account is awaiting admin approval. You will be able to sign in once it is approved.');
        }
        if (user.status === 'SUSPENDED') {
            throw new common_1.ForbiddenException('This account has been suspended. Contact support if you believe this is a mistake.');
        }
